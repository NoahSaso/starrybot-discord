...
const getStakedCW20TokenBalance = async ({
  keplrAccount,
  tokenAddress,
  network,
  extra,
  ...
  if (extra?.staking_contract) {
    const stakedTokens = await cosmClient.queryContractSmart(
      extra["staking_contract"],
      {
...
const getCW20TokenBalance = async ({
  keplrAccount,
  tokenAddress,
  network,
  extra,
...
const isCW20 = async (tokenAddress, network) => {
  ...
  try {
    ...
    // }
    if (
      (tokenInfo.token && Object.keys(tokenInfo.token).length !== 4) ||
